...
function updateCounter(){
  ...
  el.textContent = `${study.idx+1} / ${study.pool.length}`;
}

function drawCurrentCard(revealed=false){
  const c = study.pool[study.idx];
  if(!c) return;
  ...
  // Frage rendern
  const qBox = document.getElementById('questionLines');
  qBox.innerHTML='';
  ...
  // Antwort rendern
  const aBox = document.getElementById('answerLines');
  aBox.innerHTML='';
  if(revealed){
    aLines.forEach(line=>{
      const div=document.createElement('div');
...
function prevCard(){
  if(study.pool.length===0) return;
  study.idx = (study.idx - 1 + study.pool.length) % study.pool.length;
  drawCurrentCard(false);
}

function nextCard(){
  if(study.pool.length===0) return;
  study.idx = (study.idx + 1) % study.pool.length;
  drawCurrentCard(false);
}
...
(async function(){
  try{
    ...
    q.addEventListener('input', () => {}); // bewusst kein Live-Filter

    document.getElementById('flipAll').addEventListener('click', () => {
      sideSel.value = (sideSel.value === 'zh' ? 'de' : 'zh');
      study.side = sideSel.value;
      if(study.pool.length) drawCurrentCard(false);
    });
