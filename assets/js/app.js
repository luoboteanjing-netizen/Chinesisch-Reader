...
const state={ mode:'de2zh', order:'random', rateDe:0.95, pitchDe:1.0, rateZh:0.95, pitchZh:1.0,
  ...
  autoplay:{ on:false, timers:[], gapMs:800 }, settings:{ mode:'de2zh', order:'random', rateDe:0.95, pitchDe:1.0, rateZh:0.95, pitchZh:1.0, lessons:[], autoplayGap:800 },
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  // UI init
  renderModeUI(); const gr=$('#gapRange'), gv=$('#gapVal'); if(gr&&gv){ const gapSec=(state.autoplay.gapMs/1000).toFixed(1); gr.value=gapSec; gv.textContent=`(${gapSec} s)`; }
  // Excel
  setDbg(); loadExcel();
  // Handlers (minimal – Fokus Debug)
