...
/* r16-index1b: like r16-index1, but A2 label only removes the first two characters;
 * all other characters (including '*' and spaces) are preserved.
 */

let EXCEL_URL = './data/Long-Chinesisch_Lektionen.xlsx';
const DATA_START_ROW = 3; // data rows start (1-based)
...
const state={ mode:'de2zh', order:'random', rateDe:0.95, pitchDe:1.0, rateZh:0.95, pitchZh:1.0,
  lessons:new Map(), // key: lessonKey (sheet index+1) -> entries
...
function updateVoiceList(){ const box=$('#dbgVoices'); if(!box) return; box.innerHTML=''; const list=(state.voices||[]).filter(v=> state.voicePanelTarget==='zh'? isZhVoice(v) : isDeVoice(v)); if(list.length===0){ box.innerHTML='
Keine passenden Stimmen gefunden.
'; return; } list.forEach(v=>{ const row=document.createElement('div'); row.className='voice'; const name=document.createElement('div'); name.className='name'; name.textContent=v.name||'(name)'; const meta=document.createElement('div'); meta.className='meta'; meta.textContent=`${v.lang||''} ${v.default?'· default':''}`; const actions=document.createElement('div'); actions.style.marginLeft='auto'; actions.style.display='flex'; actions.style.gap='6px'; actions.style.flexWrap='wrap'; const pick=document.createElement('button'); pick.className='btn'; pick.textContent='Diese Stimme wählen'; pick.onclick=()=>{ if(state.voicePanelTarget==='zh'){ state.browserVoice.zh=v; state.settings.browserVoiceZh=v.name||v.voiceURI; } else { state.browserVoice.de=v; state.settings.browserVoiceDe=v.name||v.voiceURI; } saveSettings(); updateVoiceList(); }; const test=document.createElement('button'); test.className='btn ghost'; test.textContent='Probehören'; test.onclick=()=>{ const u=new SpeechSynthesisUtterance(state.voicePanelTarget==='zh'? '这是一个测试。' : 'Dies ist ein Test.'); u.lang=(state.voicePanelTarget==='zh')?'zh-CN':'de-DE'; u.voice=v; try{speechSynthesis.cancel();}catch(e){} speechSynthesis.speak(u); }; const act = (state.voicePanelTarget==='zh'? state.browserVoice.zh : state.browserVoice.de); if(act && (act.name===v.name || act.voiceURI===v.voiceURI)) name.textContent+=' • [Aktiv]'; actions.appendChild(pick); actions.appendChild(test); row.appendChild(name); row.appendChild(meta); row.appendChild(actions); box.appendChild(row); }); }
...
function cell(rows, rIdx0, c1){ const r = rows[rIdx0]||[]; const i = (c1|0)-1; return (i>=0 && i<r.length && r[i]!=null)? String(r[i]).trim():''; }
function makeLessonLabel(rows){ const raw = cell(rows, 1, 1); let s = raw==null? '' : String(raw); return s.length>=2 ? s.slice(2) : ''; }

async function parseExcelBuffer(buf){ const wb=XLSX.read(buf,{type:'array'}); state.lessons.clear(); for(let i=1;i<wb.SheetNames.length;i++){ const name = wb.SheetNames[i]; const sh = wb.Sheets[name]; if(!sh) continue; const rows = XLSX.utils.sheet_to_json(sh,{header:1, blankrows:false}); const r0 = DATA_START_ROW-1; const lessonKey = String(i+1); const lessonLabel = makeLessonLabel(rows); if(!state.lessons.has(lessonKey)) state.lessons.set(lessonKey, []); for(let r=r0;r<rows.length;r++){ const w_de = cell(rows,r,COL_WORD.de), w_py = cell(rows,r,COL_WORD.py), w_zh = cell(rows,r,COL_WORD.zh); const s_de = cell(rows,r,COL_SENT.de), s_py = cell(rows,r,COL_SENT.py), s_zh = cell(rows,r,COL_SENT.zh); const pos  = cell(rows,r,COL_POS); if(!(w_de||w_zh||s_de||s_zh)) continue; state.lessons.get(lessonKey).push({ word:{de:w_de, py:w_py, zh:w_zh}, sent:{de:s_de, py:s_py, zh:s_zh}, pos, sheetName:name, lessonLabel }); } } }

async function loadExcel(){ try{ const res = await fetch(EXCEL_URL); const buf = await res.arrayBuffer(); await parseExcelBuffer(buf); populateLessonSelect(); }catch(err){ alert('Excel konnte nicht geladen werden: '+err.message); } }

function populateLessonSelect(){ const sel=$('#lessonSelect'); if(!sel) return; sel.innerHTML=''; const keys=Array.from(state.lessons.keys()).map(k=>parseInt(k,10)).sort((a,b)=>a-b); for(const k of keys){ const total=state.lessons.get(String(k)).length; const seen=(state.progress.byLesson?.[String(k)]?.seen)||0; const sample = state.lessons.get(String(k))?.[0]; const label = sample?.lessonLabel || sample?.sheetName || ''; const opt=document.createElement('option'); opt.value=String(k); opt.textContent=`Lektion ${k} — ${label} (${total})` + (seen?` · ${Math.floor(seen*100/total)}% gesehen`:''); if(state.settings.lessons?.includes(String(k))) opt.selected=true; sel.appendChild(opt); } }
...
function formatZh(hz,py){ const h=(hz||'').trim(); const p=(py||'').trim(); return p? `${h}
${p}` : (h||'—'); }
function formatPinyinAndPos(py,pos){ const a=(py||'').trim(); const b=(pos||'').trim(); if(a&&b) return `${a}
${b}`; if(a) return `${a}`; if(b) return `${b}`; return ''; }

function setCard(entry){ state.current=entry; $('#solBox').classList.add('masked'); state.startedAt=Date.now(); state.revealedAt=null; const label = entry.lessonLabel||entry.sheetName||''; if(state.mode==='zh2de'){ $('#promptWord').innerHTML=(entry.word.zh||'—'); const subZh = formatPinyinAndPos(entry.word.py, entry.pos); $('#promptWordSub').innerHTML = subZh ? `${subZh} · ${label}` : label; $('#promptSent').innerHTML=formatZh(entry.sent.zh, entry.sent.py); $('#solWord').textContent=entry.word.de||'—'; $('#solSent').textContent=entry.sent.de||'—'; } else { $('#promptWord').textContent=entry.word.de||'—'; const subDe = entry.pos ? entry.pos : ''; $('#promptWordSub').textContent = subDe ? `${subDe} · ${label}` : label; $('#promptSent').textContent=entry.sent.de||'—'; $('#solWord').innerHTML=formatZh(entry.word.zh, entry.word.py); $('#solSent').innerHTML=formatZh(entry.sent.zh, entry.sent.py); } $('#btnNext').disabled=false; $('#btnReveal').disabled=false; $('#btnPlayQ').disabled=false; $('#btnPlayA').disabled=false; disableRating(); renderModeUI(); }
...
function rate(mark){ if(!state.current) return; state.session.done += 1; if(mark==='known') state.session.known += 1; else if(mark==='unsure') state.session.unsure += 1; else state.session.unknown += 1; renderSessionStats(); try{ const lessonKey = findLessonKeyOfCurrent(); if(lessonKey){ const bl=state.progress.byLesson; bl[lessonKey]=bl[lessonKey]||{seen:0}; bl[lessonKey].seen=(bl[lessonKey].seen||0)+1; localStorage.setItem(LS_KEYS.progress, JSON.stringify(state.progress)); } }catch(e){} disableRating(); nextCard(); }
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  renderModeUI();
  updateAutoplayBtn();
  const gapSec = (state.autoplay.gapMs/1000).toFixed(1);
  const gapRange=$('#gapRange'), gapVal=$('#gapVal'); if(gapRange){ gapRange.value = gapSec; } if(gapVal){ gapVal.textContent = `(${gapSec} s)`; }
  const rds=$('#rateDeRange'), rdv=$('#rateDeVal'); if(rds){ rds.value=String(state.rateDe); } if(rdv){ rdv.textContent=`(${state.rateDe.toFixed(2)})`; }
  const pds=$('#pitchDeRange'), pdv=$('#pitchDeVal'); if(pds){ pds.value=String(state.pitchDe); } if(pdv){ pdv.textContent=`(${state.pitchDe.toFixed(2)})`; }
  const rzs=$('#rateZhRange'), rzv=$('#rateZhVal'); if(rzs){ rzs.value=String(state.rateZh); } if(rzv){ rzv.textContent=`(${state.rateZh.toFixed(2)})`; }
  const pzs=$('#pitchZhRange'), pzv=$('#pitchZhVal'); if(pzs){ pzs.value=String(state.pitchZh); } if(pzv){ pzv.textContent=`(${state.pitchZh.toFixed(2)})`; }
  ...
  $('#gapRange')?.addEventListener('input', e=>{ const s=parseFloat(e.target.value)||0.8; state.autoplay.gapMs=Math.round(s*1000); state.settings.autoplayGap=state.autoplay.gapMs; const gv=$('#gapVal'); if(gv) gv.textContent = `(${s.toFixed(1)} s)`; saveSettings(); });
  $('#rateDeRange')?.addEventListener('input', e=>{ stopAutoplayOnUserAction(); state.rateDe=parseFloat(e.target.value); state.settings.rateDe=state.rateDe; const el=$('#rateDeVal'); if(el) el.textContent=`(${state.rateDe.toFixed(2)})`; saveSettings(); });
  $('#pitchDeRange')?.addEventListener('input', e=>{ stopAutoplayOnUserAction(); state.pitchDe=parseFloat(e.target.value); state.settings.pitchDe=state.pitchDe; const el=$('#pitchDeVal'); if(el) el.textContent=`(${state.pitchDe.toFixed(2)})`; saveSettings(); });
  $('#rateZhRange')?.addEventListener('input', e=>{ stopAutoplayOnUserAction(); state.rateZh=parseFloat(e.target.value); state.settings.rateZh=state.rateZh; const el=$('#rateZhVal'); if(el) el.textContent=`(${state.rateZh.toFixed(2)})`; saveSettings(); });
  $('#pitchZhRange')?.addEventListener('input', e=>{ stopAutoplayOnUserAction(); state.pitchZh=parseFloat(e.target.value); state.settings.pitchZh=state.pitchZh; const el=$('#pitchZhVal'); if(el) el.textContent=`(${state.pitchZh.toFixed(2)})`; saveSettings(); });
