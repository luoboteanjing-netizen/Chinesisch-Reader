...
/* r16 – Flashcards: load all sheets from L2 onward; show sheet name
 * Changes vs r11:
 *  - MIN_LESSON=2, MAX_LESSON=Infinity (no upper bound)
 *  - Each card stores sheetName; UI shows sheet name (lesson select + card subline)
 */

let EXCEL_URL = './data/Long-Chinesisch_Lektionen.xlsx';
const SHEET_NAME_PATTERN = /^L\s*\d{1,2}$/i;
const MIN_LESSON = 2, MAX_LESSON = Infinity;
const DATA_START_ROW = 3; // 1-based row index where data begins
const COL_WORD = { de: 1, py: 2, zh: 6 }; // 1-based col indices in Excel
...
const state = {
  mode:'de2zh',
  order:'random',
  rateDe:0.95, pitchDe:1.0,
  rateZh:0.95, pitchZh:1.0,
  lessons:new Map(), // key: lesson number string -> array of entries
...
function updateVoiceList(){ const box=$('#dbgVoices'); if(!box) return; box.innerHTML='';
  ...
  if(list.length===0){ box.textContent='\nKeine passenden Stimmen gefunden.\n'; return; }
  list.forEach(v=>{
    ...
    pick.onclick=()=>{ if(state.voicePanelTarget==='zh'){ state.browserVoice.zh=v; state.settings.browserVoiceZh=v.name||v.voiceURI; } else { state.browserVoice.de=v; state.settings.browserVoiceDe=v.name||v.voiceURI; } saveSettings(); updateVoiceList(); };
...
let _voicesRetryT;
function openVoicesPanelFor(target){ state.voicePanelTarget=target; refreshVoices(); if(!state.voices || state.voices.length===0){ clearTimeout(_voicesRetryT); let tries=0; const tick=()=>{ tries++; refreshVoices(); if(state.voices.length>0 || tries>=8) return; _voicesRetryT=setTimeout(tick,300); }; _voicesRetryT=setTimeout(tick,300); } $('#voicePanel')?.classList?.remove('hidden'); }
function closeVoices(){ $('#voicePanel')?.classList?.add('hidden'); }

function cell(row, oneBasedIdx){ const i = (oneBasedIdx|0)-1; return (Array.isArray(row) && i>=0 && i<row.length && row[i]!=null)? String(row[i]).trim() : ''; }

async function parseExcelBuffer(buf){
  const wb = XLSX.read(buf,{type:'array'});
  state.lessons.clear();
  for(const name of wb.SheetNames){
    if(!SHEET_NAME_PATTERN.test(name)) continue; // keep only L#, L ##
    const m = name.match(/\d+/); if(!m) continue;
    const n = parseInt(m[0],10);
    if(!(n>=MIN_LESSON && n<=MAX_LESSON)) continue; // from sheet 2 upwards
    const sh = wb.Sheets[name];
    const rows = XLSX.utils.sheet_to_json(sh,{header:1, blankrows:false});
    const r0 = DATA_START_ROW - 1; // convert to 0-based
    const key = String(n);
    if(!state.lessons.has(key)) state.lessons.set(key, []);
    for(let r=r0;r<rows.length;r++){
      const row = rows[r];
      const word = { de: cell(row, COL_WORD.de), py: cell(row, COL_WORD.py), zh: cell(row, COL_WORD.zh) };
      const sent = { de: cell(row, COL_SENT.de), py: cell(row, COL_SENT.py), zh: cell(row, COL_SENT.zh) };
      const pos  = cell(row, COL_POS);
      // skip empty rows
      if(!(word.de||word.zh||sent.de||sent.zh)) continue;
      const entry = { word, sent, pos, sheetName: name };
...
async function loadExcel(){
  ...
  }catch(err){ alert('Excel konnte nicht geladen werden: '+err.message); }
}

function populateLessonSelect(){ const sel=$('#lessonSelect'); if(!sel) return; sel.innerHTML='';
  const keys = Array.from(state.lessons.keys()).map(k=>parseInt(k,10)).sort((a,b)=>a-b);
  for(const k of keys){ const total = state.lessons.get(String(k)).length; const seen = (state.progress.byLesson?.[String(k)]?.seen)||0; const sample = state.lessons.get(String(k))?.[0]; const sheetLabel = sample?.sheetName ? ` — ${sample.sheetName}` : ''; const opt=document.createElement('option'); opt.value=String(k); opt.textContent = `Lektion ${k}${sheetLabel} (${total})` + (seen?` · ${Math.floor(seen*100/total)}% gesehen`:''); if(state.settings.lessons?.includes(String(k))) opt.selected=true; sel.appendChild(opt); }
}
...
function setCard(entry){ state.current=entry; $('#solBox').classList.add('masked'); state.startedAt=Date.now(); state.revealedAt=null; if(state.mode==='zh2de'){ $('#promptWord').innerHTML=(entry.word.zh||'—'); const subZh = formatPinyinAndPos(entry.word.py, entry.pos); $('#promptWordSub').innerHTML = subZh ? `${subZh} · ${entry.sheetName||''}` : (entry.sheetName||''); $('#promptSent').innerHTML=formatZh(entry.sent.zh, entry.sent.py); $('#solWord').textContent=entry.word.de||'—'; $('#solSent').textContent=entry.sent.de||'—'; } else { $('#promptWord').textContent=entry.word.de||'—'; const subDe = entry.pos ? entry.pos : ''; $('#promptWordSub').textContent = subDe ? `${subDe} · ${entry.sheetName||''}` : (entry.sheetName||''); $('#promptSent').textContent=entry.sent.de||'—'; $('#solWord').innerHTML=formatZh(entry.word.zh, entry.word.py); $('#solSent').innerHTML=formatZh(entry.sent.zh, entry.sent.py); } $('#btnNext').disabled=false; $('#btnReveal').disabled=false; $('#btnPlayQ').disabled=false; $('#btnPlayA').disabled=false; disableRating(); renderModeUI(); }
...
const START_DELAY_MS=150; const BETWEEN_DELAY_MS=800; let _ttsPrimed=false;
...
function ensurePoolForAutoplay(){ if(state.pool.length>0) return true; if(!state.settings.lessons || state.settings.lessons.length===0){ const sel=$('#lessonSelect'); const picked=[]; for(const o of sel?.selectedOptions||[]){ picked.push(o.value); } if(picked.length>0){ state.settings.lessons=picked; saveSettings(); } }
  ...
  if(state.order==='seq'){ state.idx=0; setCard(state.pool[state.idx]); } else { setCard(state.pool[Math.floor(Math.random()*state.pool.length)]); }
  return true;
}
