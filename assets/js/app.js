// r17.3.3 minimal fix: keep r17.3.1 behavior; add optional results-table + voices, guarded
let EXCEL_URL = './data/Long-Chinesisch_Lektionen.xlsx';
const COL_WORD={de:1,py:2,zh:6};
const COL_SENT={de:5,py:4,zh:7};
const COL_POS=3;
const START_ROW_1BASED=2; // 1-based
...
const state={ mode:'de2zh', order:'random', rateDe:0.95, pitchDe:1.0, rateZh:0.95, pitchZh:1.0,
  ...
  progress:{ version:'v1', cards:{}, byLesson:{} }, voices:[], trainingOn:false };

const $=s=>document.querySelector(s);
const dbg={ sheetName:'—', rows:0, lessons:0, cards:0, markerLines:0, stopReason:'' };
...
async function parseExcelBuffer(buf){ try{
  const wb=XLSX.read(buf,{type:'array'}); state.lessons.clear();
  ...
  for(let r=startRow;r<rows.length;r++){
    ...
    if(maybeName!==null){ markerCount++; if(currentKey && current && current.entries.length>0){ state.lessons.set(currentKey, current); }
      lessonIdx++; currentKey=String(lessonIdx); current={ displayName: maybeName, entries:[] }; continue; }
    ...
    current.entries.push({ word:{de:w_de, py:w_py, zh:w_zh}, sent:{de:s_de, py:s_py, zh:s_zh}, pos }); totalCards++;
  }
  ...
  populateLessonSelect(); syncUISelectionWithSettings(); safeRenderResultsTable();
 }catch(err){ console.error('parseExcelBuffer error:',err); dbg.stopReason='(Parsing-Fehler)'; setDbg(); }
}

async function loadExcel(){ try{ const res=await fetch(EXCEL_URL,{cache:'no-store'}); if(!res.ok){ throw new Error('HTTP '+res.status); } const buf=await res.arrayBuffer(); await parseExcelBuffer(buf); }catch(e){ console.error('Excel konnte nicht geladen werden:',e); dbg.stopReason='(Fetch-Fehler – prüfen: Pfad/Name/CORS)'; setDbg(); }
}

function populateLessonSelect(){ const sel=$('#lessonSelect'); if(!sel) return; sel.innerHTML=''; const keys=Array.from(state.lessons.keys()).map(k=>+k).sort((a,b)=>a-b);
  for(const k of keys){ const key=String(k); const lesson=state.lessons.get(key); if(!lesson) continue; const total=(lesson.entries||[]).length; const bl=state.progress.byLesson?.[key]||{known:0,unknown:0}; const name=lesson.displayName||`Lektion ${k}`; const opt=document.createElement('option'); opt.value=key; opt.textContent=`${name} (${total}) · Richtig ${bl.known||0} · Falsch ${bl.unknown||0}`; if(state.settings.lessons?.includes(key)) opt.selected=true; sel.appendChild(opt); }
}
...
function setCard(entry){ state.current=entry; const sol=$('#solBox'); if(sol) sol.classList.add('masked'); state.startedAt=Date.now(); state.revealedAt=null;
  if(state.mode==='zh2de'){
...
function enableButtons(on){ ['#btnPrev','#btnReveal','#btnNext','#btnPlayQ','#btnPlayA'].forEach(sel=>{ const b=$(sel); if(b) b.disabled=!on; }); disableRating(); renderModeUI(); }
...
function startTraining(){ if(!state.trainingOn){ const sel=$('#lessonSelect'); state.selectedLessons.clear(); const picked=[]; if(sel){ for(const o of sel.selectedOptions){ state.selectedLessons.add(o.value); picked.push(o.value); } }
  state.settings.lessons=picked; saveSettings(); gatherPool(); if(!state.pool.length){ alert('Bitte zuerst Lektion(en) übernehmen.'); return; } state.idx = (state.order==='seq') ? 0 : null; if(state.order==='seq') setCard(state.pool[state.idx]); else setCard(state.pool[Math.floor(Math.random()*state.pool.length)]); state.trainingOn=true; updateTrainingBtn(); } else { stopTraining(); } }
function stopTraining(){ state.trainingOn=false; updateTrainingBtn(); enableButtons(false); const sol=$('#solBox'); if(sol) sol.classList.add('masked'); ['#promptWord','#promptSent','#solWord','#solSent'].forEach(id=>{ const el=$(id); if(el) el.textContent='—'; }); const sub=$('#promptWordSub'); if(sub) sub.innerHTML='&nbsp;'; }
function updateTrainingBtn(){ const b=$('#btnStart'); if(!b) return; b.textContent = state.trainingOn? 'Training stoppen ■' : 'Training starten ▶'; }

function doReveal(){ const sol=$('#solBox'); if(sol) sol.classList.remove('masked'); state.revealedAt=Date.now(); const ttr=state.revealedAt-(state.startedAt||state.revealedAt); if(ttr>0){ state.session.ttrSum+=ttr; state.session.ttrCount+=1; } enableRating(); renderSessionStats(); }
function enableRating(){ ['#btnRateKnown','#btnRateUnsure','#btnRateUnknown'].forEach(sel=>{ const b=$(sel); if(b) b.disabled=false; }); }
function disableRating(){ ['#btnRateKnown','#btnRateUnsure','#btnRateUnknown'].forEach(sel=>{ const b=$(sel); if(b) b.disabled=true; }); }

function rate(mark){ if(!state.current) return; state.session.done += 1; if(mark==='known') state.session.known += 1; else if(mark==='unsure') state.session.unsure += 1; else state.session.unknown += 1; renderSessionStats(); try{ const lessonKey=findLessonKeyOfCurrent(); if(lessonKey){ const rec=ensureBL(lessonKey); if(mark==='known') rec.known+=1; else if(mark==='unknown') rec.unknown+=1; saveProgress(); populateLessonSelect(); safeRenderResultsTable(); } }catch(e){} disableRating(); nextCard(); }
...
// ===== Optional: Results Table (SAFE) =====
function safeRenderResultsTable(){ try{ const tbl=$('#resultsTable'); if(!tbl) return; const keys=Array.from(state.lessons.keys()).map(k=>+k).sort((a,b)=>a-b); let html='<thead><tr><th>Lektion</th><th>Richtig</th><th>Falsch</th></tr></thead><tbody>'; for(const k of keys){ const key=String(k); const name=(state.lessons.get(key)?.displayName)||`Lektion ${k}`; const bl=state.progress.byLesson?.[key]||{known:0,unknown:0}; html+=`<tr><td>${escapeHtml(name)}</td><td>${bl.known||0}</td><td>${bl.unknown||0}</td></tr>`; } html+='</tbody>'; tbl.innerHTML=html; }catch(e){ console.warn('resultsTable render skipped:', e); }
}
function escapeHtml(s){ return String(s).replace(/[&<>"']/g, m=>({"&":"&amp;","<":"&lt;",">":"&gt;",""":"&quot;","'":"&#39;"}[m])); }

// ===== Voices (SAFE, optional UI) =====
...
function openVoicesPanelFor(lang){ const panel=$('#voicePanel'); if(!panel) return; const list=$('#voicesList'); const label=$('#panelLang'); panel.classList.remove('hidden'); if(label) label.textContent=lang.toUpperCase(); const rebuild=()=>{ if(!list) return; list.innerHTML=''; refreshVoices(); const voices=(state.voices||[]).filter(v=> (v.lang||'').toLowerCase().startsWith(lang==='zh'?'zh':'de') ); if(voices.length===0){ list.innerHTML='<div class="hint">Keine Systemstimmen gefunden. Bitte klicken/tippen und „Aktualisieren“ drücken.</div>'; return; } voices.forEach(v=>{ const row=document.createElement('div'); row.className='voice'; const meta=document.createElement('div'); meta.className='meta'; meta.innerHTML=`<div class="name">${v.name}</div><div>${v.lang}${v.default?' · default':''}</div>`; const btn=document.createElement('button'); btn.className='btn small'; btn.textContent='Auswählen'; btn.addEventListener('click',()=>{ if(lang==='zh'){ state.settings.voiceZh=v.name; } else { state.settings.voiceDe=v.name; } saveSettings(); panel.classList.add('hidden'); }); row.appendChild(meta); row.appendChild(btn); list.appendChild(row); }); };
  rebuild(); const btnRefresh=$('#btnVoicesRefresh'); if(btnRefresh){ btnRefresh.onclick=rebuild; }
...
window.addEventListener('DOMContentLoaded', ()=>{
  loadSettings(); loadProgress(); state.mode=state.settings.mode||'de2zh'; state.order=state.settings.order||'random'; state.autoplay.gapMs = typeof state.settings.autoplayGap==='number' ? state.settings.autoplayGap : 800; renderModeUI(); setDbg();
  loadExcel();
  ...
  on('#btnUseLessons','click',()=>{ const sel=$('#lessonSelect'); const picked=[]; if(sel){ for(const o of sel.selectedOptions){ picked.push(o.value); } } state.settings.lessons=picked; saveSettings(); gatherPoolFromSettings(); safeRenderResultsTable(); });
  on('#btnClearLessons','click',()=>{ state.selectedLessons.clear(); state.settings.lessons=[]; saveSettings(); state.pool=[]; state.idx=null; resetSessionStats(); const sel=$('#lessonSelect'); if(sel){ for(const o of sel.options){ o.selected=false; } } if(state.trainingOn) stopTraining(); safeRenderResultsTable(); });
